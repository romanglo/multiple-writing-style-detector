...
class Medoid(object):
    ...
    def compute_kernel(self, distance):
        return min(
            self, key=lambda e: sum(distance(e, other) for other in self))

    def compute_diameter(self, distance):
        return max(distance(a, b) for a in self for b in self)


def _k_medoids_spawn_once(points,
                          k,
                          distance,
                          equality=__eq__,
    ...
    * Iteration over after 3 steps, max diameter 3
    ...
    if verbose:
        logging.debug('* New chosen kernels: {0}'.format(
            [m.kernel for m in medoids]))
    ...
    if verbose:
        logging.debug(
            '* Iteration over after {0} steps, max diameter {1}'.format(
                n, diameter))

    return diameter, medoids


def k_medoids(points,
              k,
              distance,
              spawn,
              equality=__eq__,
    ...
    diameter, medoids = min(
        (_k_medoids_spawn_once(**kw) for _ in range(spawn)), key=itemgetter(0))
    if verbose:
        logging.debug(('~~ Spawn end: min of max diameters {0:.3f} '
                       'for medoids: {1}').format(diameter, medoids))

    return diameter, medoids


def k_medoids_auto_k(points,
                     distance,
                     spawn,
                     diam_max,
                     equality=__eq__,
    ...
    for k, _ in enumerate(points, start=1):
        ...
        if diameter <= diam_max:
            break
        if verbose:
            logging.debug('*** Diameter too big {0:.3f} > {1:.3f}'.format(
                diameter, diam_max))
            logging.debug('*** Now trying {0} clusters\n'.format(k + 1))

    if verbose:
        logging.debug('*** Diameter ok {0:.3f} <= {1:.3f}'.format(
            diameter, diam_max))
        logging.debug(
            '*** Stopping, {0} clusters enough ({1} points initially)'.format(
                k, len(points)))
